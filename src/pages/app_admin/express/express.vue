            ...
            <block v-if='express && express.list'>
                <view class='item-box'>
                    <view :class="['dir-top-nowrap','item',`${index == 0 ? 'sign-text' : ''}`]" v-for='item in express.list' :key='item.id'>
                        <image v-if='index == 0' class='sign-big' src='https://cashier.qicuo.com/app/static/image/icon/order/point-green.png'></image>
                        <image v-else class='sign' src='https://cashier.qicuo.com/app/static/image/icon/order/point-gray.png'></image>
                        <view>{{item.desc}}</view>
