            ...
            <!-- #ifndef MP-BAIDU || H5 -->
                <swiper-item v-if="videoUrl && getVideo == 1 && sign !== 'lottery' && sign !== 'bargain' && sign !== 'community' && sign !== 'wholesale'">
                    <view class="swiper-img" @click="routeJumpT" v-bind:style="{backgroundImage: 'url(' + picList[0].pic_url + ')'}">
                        <image class="video-play" src="https://cashier.qicuo.com/app/static/image/video-play.png"></image>
                    </view>
                </swiper-item>
            <!-- #endif-->
        ...
        <view>
            <app-jump-button url="/pages/cart/cart" v-if="isCart">
                <view class="cart" :class="isScanQrCode ? 'bd-cart-0' : 'bd-cart-1'" >
                    <image load-lazy src="https://cashier.qicuo.com/app/static/image/icon/nav-icon-cart.png"></image>
                </view>
            </app-jump-button>
        </view>
        <view class="share-commission dir-left-nowrap cross-center" v-if="share > 0">
            <image load-lazy src="https://cashier.qicuo.com/app/static/image/icon/share/share_commission.png"></image>
            <view class="text">
                <view>分销最高可赚</view>
                <view>￥{{share}}</view>
            </view>
