					...
					<view class="u-info">
						<view class="cross-center dir-left-nowrap" :style="{'color': theme.color}">
							<view class="dir-left-nowrap">
								...
								<view class="u-price">
									<app-price v-if="is_show_price" :theme="theme" :sign="sign" :price="sellPrice" :disSelectNumber="disSelectNumber"
										:default-price="goods.price"></app-price>
								</view>
							</view>
							<app-member-mark v-if="goods.level_show === 1" :theme="theme"></app-member-mark>
						</view>
						<view class="u-stock">库存：{{stock}}</view>
					</view>
					<view class="u-close-image" @click="close">
						<image class="bd-close-image u-border-box" src="https://cashier.qicuo.com/app/static/image/icon/icon-close.png"></image>
					</view>
				</view>
				<view class="u-center">
					<scroll-view class="u-scroll-view" scroll-y="true">
						...
						<view v-if="goods.type === 'goods'" class="u-attr-group u-border-box"
							...
							<view class="u-group-name u-text">{{item.attr_group_name}}
								<view class="cycle" v-if="index===2&&isCycle">
									<view class="gray">
										每周定期发货<span class="orange">
										{{disText}}
									</span>
									</view>
							...
							<view class="dir-left-wrap">
								<view
									:class="['u-group-item', attr.select ? 'u-checked' : 'u-unchecked', attr.num_0 ? 'u-attr_num_0' : '']"
						...
						<view v-if="isCycle&&isShowBottomPrice" class="u-number dir-left-nowrap main-between cross-right">
							<view class="u-text"></view>
							<view class="dir-left-nowrap u-input-box" style="width:245rpx">
								<view>
									<span class="u-text">共{{number}}件</span><span class="u-text bottom-total-text">小计：</span>
									<span class="u-text bottom-total-price price">￥{{totalPrice}}</span>
								</view>
							</view>
						</view>
					</scroll-view>
	...
	export default {
		...
		methods: {
			...
			storeAttr: function(attr_id, group_id, num_0) {
				console.log(attr_id,group_id,num_0)
					...
					attr_list.forEach((k) => {
						if (i.attr_group_id === group_id) {
							if (k.attr_id === attr_id) {
								k.select=!k.select
								if(this.isCycle){
									this.checked.id=i.id
								}
								if(k.isNper){
									this.disSelectNumber=k.attr_name.replace("期","")
									this.totalPrice=this.goods.price*this.disSelectNumber
									
								}
								if(k.isSend){
									this.disText=k.least_time||''
								}
							} else {
								k.select = false;
							}
			...
			firstSelect: function() {
				...
				//设置初始化选中
				copyGroup.forEach((e) => {
					if (e.attr_list.length > 0) {
						e.attr_list[0].select = true
						this.disText=e.attr_list[0].least_time
					}
				})
				if(this.isShowBottomPrice){
					this.disSelectNumber= copyGroup[1].attr_list[0].attr_name?(copyGroup[1].attr_list[0].attr_name.replace("期","")):''
					//初始化小计
					this.totalPrice=this.goods.price*this.disSelectNumber
				}
				console.log("copyGroup:", copyGroup)
...
console.log(this.goods)
				...
				if(this.isCycle){
					goods.goods_list[0].id=
					goods.goods_list[0].week_key=1
					goods.use_integral=1
				}
			...
			checkLimitBuy(value) {
				if (typeof this.goods.limit_buy !== 'undefined' && this.goods.limit_buy.status == 1 && this.goods.limit_buy
					.rest_number < value) {
					this.$tips.showToast({
		...
		computed: {
			...
			sellPrice: function() {
				let showPrice = ""
				if (!this.$validation.isEmpty(this.checked)) {
					showPrice = this.goods.level_show === 1 ? this.checked.price_member : this.checked.price;
				} else if (!this.$validation.isEmpty(this.goods)) {
					showPrice = this.goods.hasOwnProperty('price_min') ? this.goods.price_min : this.goods.price;
				}
				if (this.isCycle) {
					// //周期购 价格显示
					// console.log(this.newGroup)
					// showPrice+=("x"+this.newGroup[1].attr_list.filter((e)=>e.select===true)[0].attr_name)
				}
				return showPrice;
			},
			copyGroup: function() {
				...
				let nperGroups = this.$utils.deepClone(this.goods.groups);
				let distributionGroups = this.goods.plugin && this.goods.plugin.list?this.$utils.deepClone(this.goods.plugin.list):[]
				...
				if (this.isCycle) {
					...
					let nperGroup = {
						attr_group_id: 2,
						attr_group_name: this.goods.plugin.week_custom,
						attr_list: [],
						
					}
					//
					nperGroups.forEach((e) => {
						nperGroup.attr_list.push({
							attr_id: e.group_id,
							attr_name: e.number+'期',
							pic_url: "",
							isNper:true
						})
					})
					group.push(nperGroup)
					//新增配送方式
					let distributionGroup = {
						attr_group_id: 3,
						attr_group_name: "配送时间",
						attr_list: []
					}
					console.log("distributionGroups", distributionGroups)
					distributionGroups.forEach((e) => {
						console.log(distributionGroup)
						distributionGroup.attr_list.push({
							attr_id: e.week_key+"|",
							attr_name: e.week_val,
							pic_url: "",
							least_time: e.least_time,
							isSend:true
						})
					})
					group.push(distributionGroup)
...
<style scoped lang="scss">
	...
	.u-reduced-1 {
		background-image: url("https://cashier.qicuo.com/app/static/image/icon/can-be-reduced.png");
	}

	.u-reduced-0 {
		background-image: url("https://cashier.qicuo.com/app/static/image/cart/can-be-reduced.png");
	}

	.u-added-1 {
		background-image: url("https://cashier.qicuo.com/app/static/image/cart/can-be-added.png");
	}
