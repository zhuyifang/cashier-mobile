	...
	<view class="u-list" v-show="is_show_off">
		...
		<template v-if="showCat && catList.length > 1" >
			<scroll-view scroll-x :style="{backgroundColor: catBgColor}"
				...
				<text v-for="(item, index) in catList" v-bind:key="index" v-bind:class="[
		...
		<view v-bind:class="goodsListClass" >
			...
			<template v-if="listStyle === -1 || (!listStyle && listStyle !== 0)">
				<block v-for="(goods, index) in goodsList" v-bind:key="index">
					<view v-bind:class="[goodsStyleObject, 'main-between', 'u-list-mode', 'dir-left-nowrap']"
						:style="{borderRadius:`${cBorderTop}rpx ${cBorderTop}rpx ${cBorderBottom}rpx ${cBorderBottom}rpx`}"
						hover-class="goods-hover-class" v-on:click.stop="router(goods)">
						<view class="box-grow-0 u-left">
							...
							<!-- 角标 -->
							<image v-if="showGoodsTag == 1" v-bind:class="[tagLeftTop, 'u-goods-tag']"
								v-bind:src="goodsTagPicUrl"></image>
							<!-- 商品封面图 -->
							<app-image v-bind:border-radius="`${cBorderTop}rpx ${cBorderTop}rpx ${cBorderBottom}rpx ${cBorderBottom}rpx`" width="220rpx" height="220rpx"
								v-bind:img-src="goods.cover_pic" v-bind:mode="mode"></app-image>
						</view>
						<view class="box-grow-1 u-right dir-top-nowrap main-between">
							<view v-if="(isDIY && showGoodsName) || isShowGoodsName" v-bind:class="[
							...
							<view class="box-grow-1 dir-left-nowrap cross-bottom">
								<view class="box-grow-1">
									<view v-if="isShowMemPrice(goods)" class="u-price-margin">
										<app-member-price v-bind:price="goods.level_price" :theme="theme">
										</app-member-price>
									</view>
									<view v-if="isShowVip(goods)" class="u-price-margin">
										<app-sup-vip v-bind:is_vip_card_user="goods.vip_card_appoint.is_vip_card_user"
											v-bind:discount="goods.vip_card_appoint.discount"></app-sup-vip>
									</view>
									<view v-if="showGoodsPrice || isShowOriginalPrice(goods)"
										...
										<text v-if="showGoodsPrice" class="u-goods-price"
											:style="{'color': theme.color}">{{goods.price_content}}</text>
										<text v-if="isShowOriginalPrice(goods)"
											v-bind:class="[goods.is_sales === 1 ? 'u-original-margin' : '', 'u-original-price']">{{goods.original_price}}</text>
									</view>
									<view v-if="goods.is_sales === 1 && goods.is_negotiable !== 1"
										class="u-goods-sales">{{goods.sales}}</view>
								</view>
								<template
									v-if="isShowCart && goods.goods_stock !== 0 && goods.is_negotiable !== 1 || (isDIY && isShowBuyBtn(goods))">
									<view v-if="buyBtn === 'text'" v-on:click.stop="buyProduct(goods)"
										v-bind:style="goods.buy_goods_auth ? btnStyle : disabledBtn"
										v-bind:class="[buyBtnClass, 'u-text-btn']">{{buyBtnText}}</view>
									<view v-else v-on:click.stop="buyProduct(goods)"
			...
			<template v-else-if="listStyle === 0">
				<scroll-view scroll-x class="u-swipe-left-right  box-grow-0">
					<view class="dir-left-nowrap" id="swipe-left-right">
						<block v-for="(goods, index) in goodsList" v-bind:key="index">
							<view v-bind:class="goodsStyleObject" hover-class="goods-hover-class"
							:style="{borderRadius:`${cBorderTop}rpx ${cBorderTop}rpx ${cBorderBottom}rpx ${cBorderBottom}rpx`}"
								v-on:click.stop="router(goods)" class="u-goods">
								<view class="u-top">
									<!-- 售罄 -->
									<view v-if="isShowStock(goods)" class="u-out-dialog">
										<image class="u-dialog-img" v-bind:src="sellOutPic"></image>
									...
									<image v-if="showGoodsTag == 1" v-bind:class="[tagLeftTop, 'u-goods-tag']"
										v-bind:src="goodsTagPicUrl"></image>
									<!-- 商品封面图 -->
									<app-image v-bind:border-radius="`${cBorderTop}rpx ${cBorderTop}rpx ${cBorderBottom}rpx ${cBorderBottom}rpx`" width="200rpx" height="200rpx"
										v-bind:img-src="goods.cover_pic" v-bind:mode="mode"></app-image>
								</view>
								<view class="u-content-height dir-top-nowrap main-between">
									...
									<view class="u-bottom">
										<view v-if="isShowMemPrice(goods)"
											v-bind:class="[pluginAlign, 'u-member-margin']">
											<app-member-price :theme="theme" v-bind:price="goods.level_price">
											</app-member-price>
										</view>
										...
										<view v-if="showGoodsPrice || isShowOriginalPrice(goods)"
											...
											<text v-if="showGoodsPrice" class="u-goods-price"
												:style="{'color': theme.color}">{{goods.price_content}}</text>
											<text v-if="isShowOriginalPrice(goods)"
												class="u-original-price">{{goods.original_price}}</text>
										</view>
									</view>
								</view>
							</view>
			...
			<template v-else-if="listStyle === 1">
				<block v-for="(goods, index) in goodsList" v-bind:key="index">
					<view hover-class="goods-hover-class" v-bind:class="goodsStyleObject"
					:style="{borderRadius:`${cBorderTop}rpx ${cBorderTop}rpx ${cBorderBottom}rpx ${cBorderBottom}rpx`}"
						v-on:click.stop="router(goods)" class="u-one-per-line">
						<view class="u-top">
							<!-- 售罄 -->
							<view v-if="isShowStock(goods)" class="u-out-dialog">
								<image class="u-dialog-img" v-bind:src="sellOutPic"></image>
							...
							<image v-if="showGoodsTag == 1" v-bind:class="[tagLeftTop, 'u-goods-tag']"
								v-bind:src="goodsTagPicUrl"></image>
							<!-- 商品封面图 -->
							<app-image width="100%" v-bind:img-src="goods.cover_pic" v-bind:height="coverPicHeight"
								v-bind:mode="mode" v-bind:border-radius="`${cBorderTop}rpx ${cBorderTop}rpx ${cBorderBottom}rpx ${cBorderBottom}rpx`"></app-image>
						</view>
						<view v-if="showGoodsName" v-bind:class="[textAlign, 'u-goods-name', 't-omit-two']">
							{{goods.name}}
						</view>
						<view class="u-bottom dir-left-nowrap cross-center">
							<view :style="{'color': theme.color}" class="box-grow-1 u-price">
								<view
									v-bind:class="[textStyle === 2 ? 'main-center' : '', 'dir-left-nowrap', 'cross-center']">
							...
							<template v-if="isShowBuyBtn(goods) === 1 && textStyle !== 2">
								<view v-if="buyBtn === 'text'" v-on:click.stop="buyProduct(goods)"
									...
									v-bind:style="goods.buy_goods_auth ? btnStyle : disabledBtn">{{buyBtnText}}</view>
								<view v-else v-on:click.stop="buyProduct(goods)"
									:style="{'background-color': goods.buy_goods_auth ? theme.background: '#999999'}"
									v-bind:class="['u-cart-btn-icon', 'box-grow-0', 'u-cart-btn-icon-' + buyBtn]">
								</view>
				...
				<block v-for="(goods, index) in goodsList" v-bind:key="index">
					<view hover-class="goods-hover-class" v-on:click.stop="router(goods)"
						...
						<view class="u-top box-grow-0" :style="{width: goodsStyle === 2 ? '344rpx': '346rpx'}">
							<!-- 售罄 -->
							<view v-if="isShowStock(goods)"
								:class="goodsStyle !== 2 ? 'u-out-dialog' : 'u-border-out-dialog'">
								<image :class="goodsStyle !== 2 ? 'u-dialog-img' : 'u-border-dialog-img'"
									v-bind:src="sellOutPic"></image>
							</view>
							<!-- 角标 -->
							<image v-if="showGoodsTag == 1" v-bind:class="[tagLeftTop, 'u-goods-tag']"
								v-bind:src="goodsTagPicUrl"></image>
							<!-- 商品封面图 -->
							<app-image v-bind:border-radius="`${cBorderTop}rpx ${cBorderTop}rpx ${cBorderBottom}rpx ${cBorderBottom}rpx`" :width="goodsStyle === 2 ? '342rpx': '346rpx'"
								height="346rpx" v-bind:img-src="goods.cover_pic" v-bind:mode="mode"></app-image>
						</view>
						<view
							v-bind:class="['u-bottom', 'box-grow-1', 'dir-top-nowrap', showGoodsName ? 'main-between' : 'main-right']">
							<view v-if="(isDIY && showGoodsName) || isShowGoodsName"
								v-bind:class="[textAlign, 'u-goods-name', 't-omit-two']">
								{{goods.name}}
							</view>
							<view class="box-grow-0 u-price dir-left-nowrap cross-bottom main-between">
								<view class="box-grow-1">
									<view v-if="goods.sign == 'pick' && activity"
										class="u-margin dir-left-nowrap cross-bottom">
									...
									<view v-if="goods.sign === 'flash_sale'"
										class="u-margin dir-left-nowrap cross-bottom">
										<text v-if="goods.discount_type === 1"
											:style="{'background-color': theme.background}"
											class="u-des-price">{{goods.min_discount}}折
										</text>
										<text v-if="goods.discount_type === 2"
											:style="{'background-color': theme.background}"
											class="u-des-price">减{{goods.min_discount}}元
										</text>
									</view>
									<view v-if="isShowMemPrice(goods)" v-bind:class="[pluginAlign, 'u-price-margin']">
										<app-member-price :theme="theme" v-bind:price="goods.level_price">
										</app-member-price>
									</view>
									<view v-if="isShowVip(goods)" v-bind:class="[pluginAlign, 'u-price-margin']">
										<app-sup-vip v-bind:is_vip_card_user="goods.vip_card_appoint.is_vip_card_user"
											v-bind:discount="goods.vip_card_appoint.discount"></app-sup-vip>
									</view>
									<view v-if="showGoodsPrice || isShowOriginalPrice(goods)"
										...
										<text v-if="showGoodsPrice" :style="{'color': theme.color}"
											v-bind:class="['u-goods-price', textAlign]">
											{{goods.price_content}}
										</text>
										<text v-if="isShowOriginalPrice(goods)"
											v-bind:class="[goods.is_sales === 1 ? 'u-original-margin' : '', 't-omit', 'u-original-price',  textAlign]">
											￥{{goods.original_price}}
										</text>
									</view>
									<view v-if="goods.is_sales === 1 && goods.is_negotiable !== 1"
										v-bind:class="[textAlign, 'u-goods-sales', !showGoodsPrice ? 'u-price-margin' : '']">
										{{goods.sales}}</view>
								</view>
								<template
									v-if="isShowCart && goods.goods_stock !== 0 && goods.is_negotiable !== 1 || (isDIY && isShowBuyBtn(goods) === 1 && textStyle !== 2)">
									<view v-if="buyBtn === 'text'"
										v-bind:class="[buyBtnClass, 'u-text-btn', 'box-grow-0']"
										v-bind:style="goods.buy_goods_auth ? btnStyle : disabledBtn"
										v-on:click.stop="buyProduct(goods)">{{buyBtnText}}</view>
									<view v-else
										:style="{'background-color': goods.buy_goods_auth ? theme.background: '#999999'}"
										v-bind:class="['u-cart-btn-icon', 'box-grow-0', 'u-cart-btn-icon-' + buyBtn]"
										v-on:click.stop="buyProduct(goods)"></view>
								</template>
							</view>
						</view>
					</view>
					...
					<view hover-class="goods-hover-class" v-on:click.stop="router(goods)"
						...
						<view class="u-top box-grow-0">
							...
							<image v-if="showGoodsTag == 1" v-bind:class="[tagLeftTop, 'u-goods-tag']"
								v-bind:src="goodsTagPicUrl"></image>
							<!-- 商品封面图 -->
							<app-image v-bind:border-radius="`${cBorderTop}rpx ${cBorderTop}rpx ${cBorderBottom}rpx ${cBorderBottom}rpx`" :width="goodsStyle === 2 ? '222rpx' : '224rpx'"
								height="224rpx" v-bind:img-src="goods.cover_pic" v-bind:mode="mode"></app-image>
						</view>
						<view class="u-bottom box-grow-1 dir-top-nowrap main-between">
							<view v-if="(isDIY && showGoodsName) || isShowGoodsName"
								v-bind:class="[textAlign, 'box-grow-0', 'u-goods-name', 't-omit-two']">
								{{goods.name}}
							</view>
							<view class="box-grow-1 dir-top-nowrap main-right">
								...
								<view class="box-grow-0 u-price-margin dir-left-nowrap cross-center"
									v-bind:class="[!showGoodsPrice && textStyle !== 2 ? 'main-right' : 'main-between', textStyle === 2 ? 'main-center' : '']">
									<view v-if="showGoodsPrice || isShowOriginalPrice(goods)"
										:style="{'color': theme.color}" class="u-goods-price">
										<view v-if="showGoodsPrice" v-bind:class="textAlign">{{goods.price_content}}
										</view>
										<view v-if="isShowOriginalPrice(goods)"
											v-bind:class="[textAlign, 'u-original-price']">￥{{goods.original_price}}
										</view>
										<view v-if="!isDIY && goods.is_sales === 1 && goods.is_negotiable !== 1"
											v-bind:class="['u-goods-sales']">{{goods.sales}}</view>
									</view>
									<view
									...
									<view v-if="isDIY && isShowBuyBtn(goods) === 0 && textStyle !== 2 && showBuyBtn"
										class="u-cart-btn-icon"></view>
								</view>
							</view>
						</view>
					</view>
		...
		<template v-if="isShowAttr">
			<app-attr :goods="attrGoods.goods" :previewUrl="previewUrl" :submitUrl="submitUrl"
				:attrGroupList="attrGoods.goods.attr_groups" :theme="theme" :show="attrGoods.attrShow"></app-attr>
		</template>
	</view>
</template>
		...
		computed: {
			...
			btnStyle: function() {
				...
				if (this.buyBtnStyle == 1 || this.buyBtnStyle == 3) {
					btnStyle +=
						`background-color: ${this.buttonColor ? this.buttonColor : this.theme.background};color: #ffffff;`;
				} else {
					btnStyle +=
						`border-color: ${this.buttonColor ? this.buttonColor : this.theme.background};color: ${this.buttonColor};`;
				}
				return btnStyle;
			},
...
<style scoped lang="scss">
	...
	.u-cart-btn-icon-cart {
		background-image: url('https://cashier.qicuo.com/app/static/image/icon/cats.png');
	}

	.u-cart-btn-icon-add {
		background-image: url('https://cashier.qicuo.com/app/static/image/icon/add-to.png');
	}
