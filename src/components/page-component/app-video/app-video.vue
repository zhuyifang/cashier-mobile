<template>
	<view class="app-video app-content" :style="{width: calcWidth , height: height}"   @click.stop="preventD" id="video">
		<view class="app-video app-image" @click.stop="play" v-if="!start">
			
			<image :src="picUrl" class="app-image" style="width: 100%;height:100%" :style="{
		...
		<video id="myVideo" @fullscreenchange="fullscreenChange" v-else class="app-video video" :show-mute-btn="true"
			:style="{width: width, height: height}" :autoplay="start" show-center-play-btn @error="error"
			:muted="hasAuto ? start : false" :loop="hasCycle" :src="url"></video>
...
<style scoped lang="scss">
	...
	.app-play-icon {
		width:#{130rpx};
		height: #{130rpx};
		background-image: url("https://cashier.qicuo.com/app/static/image/icon/play.png");
