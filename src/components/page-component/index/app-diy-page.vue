<template>
	<view class="app-diy-page" v-bind:class="mp_link_top ? 'mp_link_top' : ''">
		<view class="nav-bar dir-left-nowrap" v-bind:class="mp_link_top ? 'mp_link_top' : ''"
			v-if="newData.show_navs == 1">
			<block v-for="(nav, index) in newData.navs" :key="index">
				<view class="nav-bar-one cross-center box-grow-0" v-bind:class="nav_active === index ? 'active' : ''"
					:style="{'color': nav_active === index ? theme.color : ''}" v-on:click='changeNav(index)'>
					{{nav.name}}
				</view>
			</block>
		</view>
					...
					<block v-for="(temp, temp_index) in item.template.data" :key="temp_index">
						...
						<template v-else-if="temp.id === 'banner'">
							<template v-if="temp.data.banners.length > 0">
								<view
									:style="[
									{'background-color':`${temp.data.bg_padding}`,
									'padding':`${temp.data.c_padding_top + 'rpx '+ temp.data.c_padding_lr +'rpx '+ temp.data.c_padding_bottom +'rpx'}`

									}]"
									v-if="temp.data.effect == 1 || !temp.data.effect">

									<app-swiper :list="temp.data.banners"
										:autoplay="temp.data.autoplay === 0 ? true : false" name="pic_url"
										:effect3d="temp.data.style === 2 ? true : false" :height="temp.data.height"
										:effect3dPreviousMargin="50"
										:cBorderBottom="temp.data.c_border_bottom"
										:cBorderTop="temp.data.c_border_top"
										...
										:mode="temp.data.mode"></app-swiper>
								</view>
								<view
									:style="[
									{'background-color':`${temp.data.bg_padding}`,'padding':`${temp.data.c_padding_top + 'rpx '+ temp.data.c_padding_lr +'rpx '+ temp.data.c_padding_bottom +'rpx'}`}]"
									v-if="temp.data.effect == 2">
									<u-swiper :autoplay="temp.data.autoplay===0?true:false"
										:borderRadius="100"
										:cBorderBottom="temp.data.c_border_bottom" :cBorderTop="temp.data.c_border_top"
										:duration="temp.data.duration?temp.data.duration:500" :height="temp.data.height"
										:imgMode="temp.data.fill===0?'aspectFit':'scaleToFill'"
										:interval="temp.data.interval?temp.data.interval:3000" :list="temp.data.banners"
										mode="temp.data.mode" name="pic_url"
						...
						<template v-else-if="temp.id === 'goods'">
							...
							'padding':`${temp.data.c_padding_top + 'rpx '+ temp.data.c_padding_lr +'rpx '+ temp.data.c_padding_bottom +'rpx'}`
								...
								<template v-if="temp.data.catPosition === 'top'">
									<u-ordinary-list v-bind:theme="theme" v-bind:show-cat="temp.data.showCat"
										v-bind:cat-style="temp.data.catStyle" v-bind:cat-list="temp.data.catList"
										v-bind:list-style="temp.data.listStyle" v-bind:list="temp.data.list"
										...
										v-bind:buy-btn="temp.data.buyBtn" v-bind:show-buy-btn="temp.data.showBuyBtn"
										...
										v-bind:is-under-line-price="temp.data.isUnderLinePrice" v-bind:isDIY="true"
										v-bind:fill="temp.data.fill" v-bind:tag-color="temp.data.tagColor"
										v-bind:cat-selected-color="temp.data.catSelectedColor"
										v-bind:cat-unselected-color="temp.data.catUnselectedColor"
										v-bind:cat-bg-color="temp.data.catBgColor" v-on:buyProduct="buyProduct"
										v-bind:bg="temp.data.bg" v-bind:cBorderBottom="temp.data.c_border_bottom"
										v-bind:cPaddingLr="temp.data.c_padding_lr"
										v-bind:cBorderTop="temp.data.c_border_top"
										v-bind:catPosition="temp.data.catPosition"
										v-bind:customizeGoodsTag="temp.data.customizeGoodsTag" v-bind:sign="temp.id"
										v-bind:tagColor="temp.data.tagColor">
									</u-ordinary-list>
								</template>
								<template v-else>
									<app-recommended-product-list v-bind:show-cat="temp.data.showCat"
										v-bind:show-buy-btn="temp.data.showBuyBtn" v-bind:cat-style="temp.data.catStyle"
										v-bind:cat-list="temp.data.catList" v-bind:list="temp.data.list"
										...
										v-bind:goods-tag-pic-url="temp.data.goodsTagPicUrl"
										v-bind:button-color="temp.data.buttonColor"
										v-bind:is-under-line-price="temp.data.isUnderLinePrice" v-bind:theme="theme"
										v-bind:cat-position="temp.data.catPosition" v-on:buyProduct="buyProduct"
										v-bind:tag-color="temp.data.tagColor"
										v-bind:cat-selected-color="temp.data.catSelectedColor"
										v-bind:cat-unselected-color="temp.data.catUnselectedColor"
										v-bind:cat-bg-color="temp.data.catBgColor"></app-recommended-product-list>
								</template>
							</view>
						</template>
						<!-- 插件商品列表 -->
						<template
							v-else-if="['miaosha', 'flash-sale'].indexOf(temp.id) !== -1 && temp.data.addGoodsType == 0">
							<app-diy-m-goods-list v-bind:mTitle="temp.data.mTitle" v-bind:mColor="temp.data.mColor"
								v-bind:mBgType="temp.data.mBgType" v-bind:mBgColor="temp.data.mBgColor"
								v-bind:mBgGradientColor="temp.data.mBgGradientColor"
								v-bind:mTimeColor="temp.data.mTimeColor" v-bind:mTimeBgColor="temp.data.mTimeBgColor"
								...
								v-bind:showProgressBar="temp.data.showProgressBar" v-bind:mData="temp.data.mData"
								v-bind:sign="temp.id" v-bind:theme="theme" v-bind:bg="temp.data.bg"
								v-bind:cBorderBottom="temp.data.c_border_bottom"
								v-bind:cBorderTop="temp.data.c_border_top"
								v-bind:cPaddingBottom="temp.data.c_padding_bottom"
								v-bind:cPaddingTop="temp.data.c_padding_top" v-bind:cPaddingLr="temp.data.c_padding_lr">
						...
						<template v-else-if="temp.id === 'pintuan'
                            ...
                            'background-repeat':`${transLabelBackgroundRepeat(temp.data.mode)}`}]">
								<template v-if="temp.id === 'integral-mall' && temp.data.showCoupon">
									<app-exclusive-coupon v-bind:none-color="true" v-bind:list="temp.data.coupon_list"
										v-bind:sign="temp.id" v-bind:text-color="temp.data.couponColor"
										v-bind:show-top="false" v-bind:coupon_req="coupon_req"
										v-bind:unclaimed-bg="temp.data.couponPicUrl"></app-exclusive-coupon>
								</template>
								<app-diy-goods-list v-bind:show-buy-btn="temp.data.showBuyBtn"
									v-bind:list="temp.data.list" v-bind:list-style="temp.data.listStyle"
									v-bind:goods-cover-proportion="temp.data.goodsCoverProportion"
									v-bind:fill="temp.data.fill" v-bind:goods-style="temp.data.goodsStyle"
									v-bind:show-goods-name="temp.data.showGoodsName"
									v-bind:text-style="temp.data.textStyle" v-bind:sign="temp.id"
									v-bind:button-color="temp.data.buttonColor"
									v-bind:buy-btn-style="temp.data.buyBtnStyle"
									v-bind:buy-btn-text="temp.data.buyBtnText"
									v-bind:show-goods-tag="temp.data.showGoodsTag" v-bind:page-hide="pageHide"
									v-bind:show-progress-bar="temp.data.showProgressBar"
									v-bind:customize-goods-tag="temp.data.customizeGoodsTag"
									v-bind:is-under-line-price="temp.data.isUnderLinePrice"
									v-bind:goods-tag-pic-url="temp.data.goodsTagPicUrl" v-bind:theme="theme"
									v-bind:bg="temp.data.bg" v-bind:cBorderBottom="temp.data.c_border_bottom"
									v-bind:cBorderTop="temp.data.c_border_top"
									v-bind:cPaddingLr="temp.data.c_padding_lr">
								</app-diy-goods-list>
						...
						<template v-else-if="temp.id === 'rubik'">
							<view style="position: relative">
								<app-image-ad v-bind:image-style="temp.data.style" v-bind:list="temp.data.list"
									v-bind:height="temp.data.height"></app-image-ad>
						...
						<template v-else-if="temp.id === 'notice'">
							<u-announcement v-bind:bgColor="temp.data.background" v-bind:btn-color="temp.data.btnColor"
								v-bind:btn-height="temp.data.btnHeight" v-bind:btn-radius="`${temp.data.btnRadius}rpx`"
								v-bind:btn-text="temp.data.btnText" v-bind:btn-text-color="temp.data.btnTextColor"
								v-bind:btn-width="temp.data.btnWidth" v-bind:content="temp.data.content"
								v-bind:header-url="temp.data.headerUrl" v-bind:icon="temp.data.icon"
								v-bind:name="temp.data.name" v-bind:textColor="temp.data.textColor"></u-announcement>
						</template>
						<template v-else-if="temp.id === 'nav'">
							<!-- 新数据 -->
							<template v-if="temp.data.modeType">
								<app-navigation-two-icon v-bind:mode-type="temp.data.modeType"
									v-bind:background="temp.data.backgroundColor" v-bind:color="temp.data.color"
									v-bind:navs="temp.data.navs" v-bind:columns="temp.data.columns"
									v-bind:temp-index="temp_index" v-bind:nav-type="temp.data.navType"
									v-bind:alone-num="temp.data.aloneNum" v-bind:line-num="temp.data.lineNum"
									v-bind:swiper-type="temp.data.swiperType"
									v-bind:swiper-color="temp.data.swiperColor"
									v-bind:swiper-no-color="temp.data.swiperNoColor" :bg-type="temp.data.bgType"
							...
							<template v-else>
								<view :style="[
                                        ...
                                        'background-position':`${transLabelBackgroundPosition(temp.data.position)}`,
                                        'background-repeat':`${transLabelBackgroundRepeat(temp.data.mode)}`}]">
									<app-navigation-icon v-bind:background="''" v-bind:color="temp.data.color"
										v-bind:rows="temp.data.rows" v-bind:columns="temp.data.columns"
										v-bind:scroll="temp.data.scroll" v-bind:navs="temp.data.navs">
									</app-navigation-icon>
								</view>
							</template>
						</template>
						<template v-else-if="temp.id === 'video'">
							<view
								...
								'padding':`${temp.data.c_padding_top + 'rpx '+ temp.data.c_padding_lr +'rpx '+ temp.data.c_padding_bottom +'rpx'}`}]"
								v-if="temp.data.effect == 1 || !temp.data.effect">
								<app-video v-bind:pic-url="temp.data.pic_url" v-bind:url="temp.data.url"
									v-bind:has-auto="temp.data.hasAuto" v-bind:has-cycle="temp.data.hasCycle"
									v-bind:bgPadding="temp.data.bg_padding"
									v-bind:cBorderBottom="temp.data.c_border_bottom"
									v-bind:cBorderTop="temp.data.c_border_top"
									v-bind:cPaddingBottom="temp.data.c_padding_bottom"
									v-bind:cPaddingLr="temp.data.c_padding_lr"
									v-bind:cPaddingTop="temp.data.c_padding_top">
								</app-video>
							</view>
						</template>
						<template v-else-if="temp.id === 'link'">
							<app-associated-link v-bind:arrows-switch="temp.data.arrowsSwitch"
								v-bind:background="temp.data.background" v-bind:color="temp.data.color"
								v-bind:position="temp.data.position" v-bind:style-color="temp.data.styleColor"
								v-bind:link="Object.assign({}, temp.data.link)" v-bind:styleNum="temp.data.style"
								v-bind:pic-switch="temp.data.picSwitch" v-bind:pic-url="temp.data.picUrl"
								v-bind:font-size="temp.data.fontSize" v-bind:title="temp.data.title">
							</app-associated-link>
						</template>
						<template v-else-if="temp.id === 'topic'">
							<template v-if="temp.data.style === 'normal'">
								<app-special-topic v-bind:count="temp.data.count" v-bind:icon="temp.data.icon"
									v-bind:logo_1="temp.data.logo_1" v-bind:logo_2="temp.data.logo_2"
									v-bind:topic_list="temp.data.topic_list"></app-special-topic>
							</template>
							<template v-else-if="temp.data.style === 'list'">
								<app-special-topic-list v-bind:cat-show="temp.data.cat_show"
									v-bind:list="temp.data.list" v-bind:theme="theme"
									v-bind:topic-list="temp.data.topic_list" v-bind:tag-color="temp.data.tagColor"
									v-bind:cat-selected-color="temp.data.catSelectedColor"
									v-bind:cat-unselected-color="temp.data.catUnselectedColor"
									v-bind:cat-bg-color="temp.data.catBgColor"></app-special-topic-list>
							</template>
						</template>
						<template v-else-if="temp.id === 'store'">
							<app-shop v-bind:list="temp.data.list" v-bind:nav-pic-url="temp.data.navPicUrl"
								v-bind:score-pic-url="temp.data.scorePicUrl" v-bind:show-name="temp.data.showName"
								v-bind:show-score="temp.data.showScore" v-bind:show-tel="temp.data.showTel"
								v-bind:scrollTop="scrollTop"></app-shop>
						</template>
						<template v-else-if="temp.id === 'copyright'">
							<app-copyright v-bind:background-color="temp.data.backgroundColor"
								v-bind:text="temp.data.text" v-bind:pic-url="temp.data.picUrl"
								v-bind:link="temp.data.link"></app-copyright>
						</template>
						<template v-else-if="temp.id === 'user-info'">
							<app-user-center v-bind:top-pic-url="temp.data.backgroundPicUrl"
								v-bind:member-pic-url="userCenter.member_pic_url" v-bind:top-style="temp.data.style">
							</app-user-center>
						</template>
						<template v-else-if="temp.id === 'user-order'">
							<app-my-order v-bind:order_bar="temp.data.navs"
								v-bind:background-color="temp.data.backgroundColor" v-bind:theme="theme"></app-my-order>
						</template>
						<!-- 优惠券 -->
						<template v-else-if="temp.id === 'coupon'">
							<app-exclusive-coupon-two background="#FFFFFF" v-bind:list="temp.data.coupon_list"
								v-bind:receive-bg="temp.data.receiveBg" v-bind:text-color="temp.data.textColor"
								v-bind:unclaimed-bg="temp.data.unclaimedBg" v-bind:coupon_req="coupon_req"
								v-bind:coupon-bg="temp.data.couponBg ? temp.data.couponBg : `#D9BC8B`"
								v-bind:coupon-bg-type="temp.data.couponBgType" v-bind:add-type="temp.data.addType"
								v-bind:page_id="page_id" v-bind:template_id="Number.parseInt(item.template_id)"
								v-bind:d-index="[nav_active,temp_index]" v-bind:m-index="dIndex" v-bind:d-type="dType">
							</app-exclusive-coupon-two>
						</template>
						<template v-else-if="temp.id === 'timer'">
							<app-diy-timer v-bind:pic-url="temp.data.picUrl"
								v-bind:start-date-time="temp.data.startDateTime"
								v-bind:end-date-time="temp.data.endDateTime" v-bind:page-hide="pageHide"
								v-bind:bg-pic-url="temp.data.bgPicUrl" v-bind:link="temp.data.link"></app-diy-timer>
						</template>
						<template v-else-if="temp.id === 'mch'">
							<view :style="[
                                ...
                                'background-position':`${transLabelBackgroundPosition(temp.data.position)}`,
                                'background-repeat':`${transLabelBackgroundRepeat(temp.data.mode)}`}]">
								<app-goods-shop v-bind:card-style="temp.data.cardStyle"
									v-bind:background-color="temp.data.backgroundColor" type="diy" v-bind:theme="theme"
									v-bind:mch_list="temp.data.list" v-bind:coupon_req="coupon_req"
									v-bind:is_required="is_required" v-bind:show-goods="temp.data.showGoods">
								</app-goods-shop>
							</view>
						</template>
						<template v-else-if="temp.id === 'empty'">
							<app-empty v-bind:height="temp.data.height" v-bind:background-color="temp.data.background">
							</app-empty>
						</template>
						<template v-else-if="temp.id === 'ad'">
							<app-ad v-bind:unit-id="temp.data.id" v-bind:type="temp.data.type"
								v-bind:coupon-url="coupon_url" v-bind:video-url="temp.data.video_url"
								v-bind:pic-url="temp.data.pic_url" v-bind:coupon-params="item | getObje"></app-ad>
						</template>
						<template v-else-if="temp.id === 'modal'">
							<app-popup-ad v-bind:opened="temp.data.opened" v-bind:times="temp.data.times"
								v-bind:list="temp.data.list" v-bind:multiple="true" v-bind:is_storage="is_storage"
						...
						<template v-else-if="temp.id === 'quick-nav' && temp.data.navSwitch == 1">
							<app-quick-navigation v-bind:value="temp.data"
								v-bind:use-mall-config="temp.data.useMallConfig"></app-quick-navigation>
						</template>
						<!-- #ifdef MP-WEIXIN -->
						<template v-else-if="temp.id === 'mp-link'">
							<view v-bind:class="temp.data.position === 'top' ? `mp-link-top` : ``"
								...
								<official-account v-on:load="mpLink(temp.data.position)" v-on:error="mpLinkError">
								</official-account>
							</view>
						</template>
						<!-- #endif -->
						...
						<template v-else-if="temp.id === 'map'">
							<app-map v-bind:background-pic-url="temp.data.backgroundPicUrl"
								v-bind:height="temp.data.height" v-bind:latitude="temp.data.latitude"
								v-bind:longitude="temp.data.longitude" v-bind:margin-top="temp.data.marginTop"
								v-bind:margin-top-color="temp.data.marginTopColor" v-bind:padding-x="temp.data.paddingX"
								v-bind:padding-y="temp.data.paddingY"></app-map>
						</template>
						<!-- #endif -->
						<template v-else-if="temp.id === 'check-in'">
							<app-check-in v-bind:background-pic-url="temp.data.backgroundPicUrl"
								v-bind:hotspot="temp.data.hotspot" v-bind:show-text="temp.data.showText"
								v-bind:text-color="temp.data.textColor" v-bind:text-position="temp.data.textPosition">
							</app-check-in>
						</template>
						...
						<template v-else-if="temp.id === 'form'">
							<app-diy-form v-bind:title="temp.name" v-bind:list="temp.data.formDialog"
								v-bind:item-padding-x="temp.data.inputPadding"
								v-bind:item-margin-y="temp.data.marginBottom" v-bind:item-height="temp.data.height"
								v-bind:show-item-border="false"
								v-bind:label-position="transLabelPosition(temp.data.style)"
								v-bind:label-color="temp.data.inputLabel" label-text-align="left"
								v-bind:show-input-border="true" v-bind:input-background="temp.data.inputBackground"
								...
								v-bind:input-radius="temp.data.radius" v-bind:show-submit="true"
								v-bind:submit-url="diyFormSubmitUrl" v-bind:submit-btn-text="temp.data.borderContent"
								...
								v-bind:margin-top="temp.data.marginTop" v-bind:margin-color="temp.data.marginColor"
								v-bind:padding-top="temp.data.padding" v-bind:padding-bottom="temp.data.padding"
								v-bind:select-box-color="temp.data.selectBoxColor"></app-diy-form>
						</template>
						<!--超级会员卡组件-->
						<template v-else-if="temp.id === 'vip-card'">
							<view class="vip-card" :style="[
                            {
								'background-color':`${temp.data.background}`,

								'padding':`${temp.data.c_padding_top+'rpx '+temp.data.c_padding_lr+'rpx '+temp.data.c_padding_bottom+'rpx'}`
							}]">
								<app-vip-card v-bind:value="temp.data" top="0" v-bind:background="temp.data.background"
									v-bind:cBorderBottom="temp.data.c_border_bottom"
									v-bind:cBorderTop="temp.data.c_border_top">
								</app-vip-card>
							</view>
						...
						<template v-else-if="temp.id === 'module' && is_module">
							<app-module v-bind:tab-type="temp.data.tabType" v-bind:tab-color="temp.data.tabColor"
								v-bind:text-color="temp.data.textColor" v-bind:tab-background="temp.data.tabBackground"
								v-bind:list="temp.data.list" v-bind:d-index="[index, temp_index]" v-bind:theme="theme"
								v-bind:page-hide="pageHide" v-bind:home-pages="homePages" v-bind:is_storage="is_storage"
								v-bind:page_id="page_id" v-bind:is_required="is_required" v-bind:coupon_req="coupon_req"
								@buyProduct="buyProduct"></app-module>
						</template>
		...
		computed: {
			...
			bottomHeight() {
				let s = this.tabBarNavs.find(item => {
					return item.url.indexOf('/pages/index/index') !== -1 && item.url.indexOf(
...
<style scoped lang="scss">
	...
	//#ifdef MP-ALIPAY
	.app-diy-page ::v-deep .app-diy-page {
