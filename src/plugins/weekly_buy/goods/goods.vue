<template>
	<app-layout>
		...
		<template v-else>
			<!--商品轮播图-->
			<app-goods-banner :picList="picUrl" :isCart="false" :share="goods.share" :goods_id="goodsId"
				:video-url="goods.video_url" sign="booking"></app-goods-banner>
			<view>
				<bd-name :theme="getTheme" :name="goods.name" :is-negotiable="goods.is_negotiable"
					:subtitle="goods.subtitle" :level-show="0" :price="goods.price"
					:original-price="goods.original_price" :price-max="goods.price_max" :price-min="goods.price_min"
					:price-member-max="goods.price_member_max" :price-member-min="goods.price_member_min"
					:discount='discount' :is-vip-card-user="is_vip_card_user" :sales="goods.sales" :unit="goods.unit"
					:is-sales="goods.is_sales" :is-vip="false" :is-process="true" :goods-id="goods.id"
					:extra-quick-share="goods.extra_quick_share" :app-share-pic="goods.app_share_pic"
					:app-share-title="goods.app_share_title" :poster-config="poster_config"
					:poster-generate="poster_generate" :has-poster-nav="true" v-bind:goods="goods"
					@share="hShareAppMessage" :min-number="goods.min_number" :limit-buy="goods.limit_buy"></bd-name>
			</view>
			<bd-coupon @change="setCoupon" :theme="getTheme" :coupons="goods.goods_coupon_center"></bd-coupon>
			<bd-xbc :coAttr="is_open" :attr-list="checked && checked.attr_list" :type="goods.type"
				...
				:attr-groups="goods.attr_groups" :goods-stock="goods.goods_num" @openAttr="open"></bd-xbc>
			...
			<bd-kb :limit="goods.goods_marketing && goods.goods_marketing.limit" :express="goods.express"
				:shipping="goods.goods_marketing && goods.goods_marketing.shipping"
				:pickup="goods.goods_marketing && goods.goods_marketing.pickup"></bd-kb>
			<view class="bd-store" v-if="store[0]">
				<app-store :store_id="store[0] && store[0].id" :address="store[0] && store[0].address"
					:name="store[0].name" :business_hours="store[0].business_hours" :storeNum="storeNum"
					:goods_id="goodsId"></app-store>
			</view>
			<bd-comments :goods-id="goods.id"></bd-comments>
			<bd-detail :detail="goods.detail"></bd-detail>
			<!--相关推荐-->
			<app-related-suggestion-product :theme="getTheme" sign="booking" :list="list">
			...
			<view v-if="is_open == 1" class="safe-area-inset-bottom u-bottom-fixed">
				...
				<view class="bd-bottom dir-left-nowrap cross-center">
					<view class="u-back dir-top-nowrap main-center cross-center box-grow-0"
						@click="router('/pages/index/index')">
						<image class="u-icon" src="https://cashier.qicuo.com/app/static/image/icon/index.png"></image>
						<text class="u-text">首页</text>
					</view>
					<bd-service :name="goods.name" :url="webUrl"></bd-service>
					<view v-if="goods.goods_num > 0"
						:style="{'background': goods.buy_goods_auth ? getTheme.background_gradient_btn : '#9999999'}"
						class="u-btn box-grow-1" @click="open">
						立即购买
					</view>
					<view v-else class="u-btn u-oversell-btn box-grow-1">已售罄</view>
				</view>
			</view>
			<u-attr :goods="goods" v-model="show" :theme="getTheme" :checked="checked" :is-cycle="true" @check="check"
				:is_show_left="false" :isShowBottomPrice="true" right-text="立即购买" :right-func="true"
				@rightFunc="rightFunc">
			</u-attr>
			<!-- 快捷导航 -->
	...
	export default {
		...
		methods: {
			...
			rightFunc(data) {
				...
				// data=test
				uni.navigateTo({
					url: `/pages/order-submit/order-submit?mch_list=${JSON.stringify([data])}&preview_url=${encodeURIComponent(this.$api.book.order_preview.replace("plugin/booking/api/order/order-preview","plugin/weekly_buy/api/order/preview"))}&submit_url=${encodeURIComponent(this.$api.book.order_submit.replace("plugin/booking/api/order/submit","plugin/weekly_buy/api/order/submit"))}&plugin=weekly_buy`
				});
			},
			newRecommend(id) {
				this.$request({
