        ...
        <view class="list" v-if="list && list.length > 0">
            <view @click="toOrder(item.nickname)" v-for="item in list" :key="item.id">
                <view class="main-between cross-center item">
                    ...
                    <view>
                        <text>订单</text>
                        <image class="arrow-right" src='https://cashier.qicuo.com/app/static/image/icon/arrow-right.png'></image>
                    </view>
                </view>
            </view>
        </view>
        <view class='no-tip' v-if="list && list.length == 0">
            <image src="https://cashier.qicuo.com/app/static/image/user-default-avatar.png"></image>
            <span>暂无队员</span>
        </view>
    </app-layout>
</template>
