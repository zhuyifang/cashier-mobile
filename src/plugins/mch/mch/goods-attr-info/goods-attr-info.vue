    ...
    <app-layout>
        <view class="group main-center cross-center">
            <view @click="choose(item,index)" v-for="(item,index) in attr_groups" :key="index" class="box-grow-1 group-item main-center cross-center">
                ...
                <image v-if="showGroup && groupIndex == index" class="more" :src="item.choose ? './../image/close.png' :'https://cashier.qicuo.com/app/static/image/icon/icon-up.png'"></image>
                <image class="more" v-else :src="item.choose ? './../image/open.png' :'https://cashier.qicuo.com/app/static/image/icon/icon-down.png'"></image>
            </view>
        </view>
               ...
               <template v-if="startIndex < item.showIndex && item.showIndex < endIndex">
                   <view class="attr-name">
                       <view class="check" @click="unSelect(item,index)">
                           <image src="https://cashier.qicuo.com/app/static/image/icon/icon-uncheck.png" v-if="!item.select"></image>
                           <image src="https://cashier.qicuo.com/app/static/image/icon/icon-checkbox-checked-a.png" v-else></image>
                       </view>
        ...
        <view :class="['bottom', 'cross-center', 'dir-left-nowrap', `${iphone_x? 'iphone_x':''}`]">
            <view @click="allSelect" class="check">
                <image src="https://cashier.qicuo.com/app/static/image/icon/icon-uncheck.png" v-if="!selectStatus"></image>
                <image src="https://cashier.qicuo.com/app/static/image/icon/icon-checkbox-checked-a.png" v-else></image>
            </view>
            <view>全选</view>
